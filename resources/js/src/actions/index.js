  ...
  return async (dispatch) => {
    try {
      ...
      console.log("hello3");
       ...
       var balance = await Tezos.tz.getBalance(userAddress);
...
/*
export const transferTokens = async (contractAddress, recipientAddress, tokenId, amount) => {
  return async (dispatch) => {
    try {
      const contract = await Tezos.contract.at(contractAddress);
      const transferOperation = await contract.methods.transfer([
        { from_: senderAddress, txs: [{ to_: recipientAddress, token_id: tokenId, amount: amount.toString() }] },
      ]).send();
...
//edskRkDnsA64eDc5h4TSCfZPafjwckiHFLuYvGCxgKPD6yw2CFgJFDFk1Y8mZ1EwLrwa2QvJeM3hntrsxXKkyHre8szkRD3bUd

export const transferTokens = ({ Tezos, senderAddress, recipientAddress, tokenId, amount }) => {
  return async (dispatch) => {
    try {
      const contract = await Tezos.wallet.at(config.tokenAddress);


      const balanceRequest = [
        { owner: senderAddress, token_id: tokenId },
      ];
      const balanceResponse = await contract.views.balance_of(balanceRequest).read();
      console.log(balanceResponse[0].balance.toNumber().toString());
      console.log(contract.views.balance_of(balanceRequest));

      const transferOperation = await contract.methods.transfer([
        { from_: senderAddress, txs: [{ token_id: tokenId, amount: '0', to_: recipientAddress }] },
      ]).send({ sender: senderAddress });
